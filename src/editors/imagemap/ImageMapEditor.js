...
class ImageMapEditor extends Component {
	...
	componentDidMount() {
		...
		let result=	params.get('myparam')
		console.log(result,"this.props.location.search==>")
		this.getS3fileUrl(result)
	...
	getS3fileUrl=(url)=>{
		...
		//   this.showLoading(false);
		this.handlers.onUpload(json.objects)
	...
	handlers = {
		...
		onImport: files => {
			...
			this.canvasRef.handler.importJSON(this.state.s3Url);
			this.showLoading(false);
			// if (files) {
			// 	this.showLoading(true);
			// 	setTimeout(() => {
			...
			// 				console.log(this.state.s3Url,"this.state.s3Url")
			// 				if(this.state.s3Url){
		...
		onUpload: (jsonFile) => {
			...
			this.handlers.onImport(jsonFile);
			console.log("upload==>")
